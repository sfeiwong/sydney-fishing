...
```jsonc
{
  ...
  "lon": 151.21,           // 钓点经度（必填）
  "sheltered": true,       // 是否为内湾/避风点（影响安全评估逻辑）
  "tide_delay": 10,        // 相对 Fort Denison 的潮汐时间偏差（分钟，可负）
...
| 条件 | 结果 |
|------|------|
| 外海点（`sheltered=False`）且 swell > 1.3m 或 wind > 20km/h | ❌ 极度危险（红色） |
| 内湾点（`sheltered=True`）且 swell > 1.4m 或 wind > 22km/h | ⚠️ 谨慎前往（橙色） |
| 其他 | ✅ 极力推荐（绿色） |
...
`services/weather.py` 将坐标取整到 0.5°（约 55km 精度）再作为缓存键，避免 30 个钓点发起 30 次独立 API 请求。同一区域的钓点共用同一份预报数据。缓存 TTL = 1 小时（`config.py → WEATHER_CACHE_TTL`）。
...
**新增钓点**
→ 直接在 `data/spots.json` 末尾追加一个符合上述结构的 JSON 对象，确保 `fish_tags` 和 `supported_methods` 使用标准化枚举值。
